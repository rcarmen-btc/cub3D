...
{	
	if (set->kfl.left == 1)
	{
		if ((set->pattr.fpa -= set->ray.angle10 / 4) < set->ray.angle0)
			set->pattr.fpa+=set->ray.angle360;
	}
	if (set->kfl.right == 1)
	{
		if ((set->pattr.fpa += set->ray.angle10 / 4) >= set->ray.angle360)
			set->pattr.fpa -= set->ray.angle360;
	}
	set->pattr.playerxd=set->tabs.fcost[set->pattr.fpa];
	set->pattr.playeryd=set->tabs.fsint[set->pattr.fpa];
	...
	set->pattr.fpseed)) / 64))][((set->pattr.fpx + 
	(int)(set->pattr.playerxd * set->pattr.fpseed)) / 64)] != '1')
	{
